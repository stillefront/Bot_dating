    ...
    io.on('connection', function (socket) {
        ...
        socket.on('message', async function (message) {
            ...
            async function getBot1Auth() {
                try {
                    botAuth1[socket.id] = await Bot_basic.findOne({ 'name': clientSearch[socket.id].bot1 },).exec();
            ...
            async function getBot2Auth() {
                try {
                    botAuth2[socket.id] = await Bot_basic.findOne({ 'name': clientSearch[socket.id].bot2 },).exec();
